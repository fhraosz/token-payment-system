...
class PaymentApiTest extends Specification {
    ...
    def "토큰 유효성 검사 API 호출 테스트"() {
        ...
        paymentApi.tokenizationServiceUrl = "http://localhost:8080"

        when:
        Boolean result = paymentApi.validateToken(requestDto)

        then:
        1 * restClient.postRequest("http://localhost:8080/validate", requestDto) >> true
        result == true
    }
    ...
    def "토큰 상태 변경 API 호출 테스트"() {
        ...
        then:
        1 * restClient.postRequest("http://localhost:8080/change/state", requestDto) >> true
        result == true
    }
    ...
    def "토큰 유효성 검사 실패 테스트"() {
        ...
        then:
        1 * restClient.postRequest("http://localhost:8080/validate", requestDto) >> false
        result == false
    }
    ...
    def "토큰 상태 변경 실패 테스트"() {
        ...
        then:
        1 * restClient.postRequest("http://localhost:8080/change/state", requestDto) >> false
        result == false
    }
}
